...
export async function findExistingAccount(email?: string, phone?: string): Promise<ExistingAccountMatch | null> {
  if (email) {
    const byEmail = await User.findOne({ email: email.toLowerCase().trim(), deletedAt: null });
    if (byEmail) return { user: byEmail, matchedBy: 'email' };
  }
  const digits = digitsOf(phone);
  if (digits.length >= 10) {
    const tail = digits.slice(-10);
    // Exact last-10 match only (avoid loose substring false positives).
    const escaped = tail.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
    const candidates = await User.find({
      deletedAt: null,
      phone: { $exists: true, $ne: '', $regex: escaped },
    }).limit(100);
    const byPhone =
      candidates.find(u => digitsOf(u.phone).slice(-10) === tail) ?? null;
